...
export interface UpdateDigitalTwinParams extends Partial<CreateDigitalTwinParams> {
    avatar_url?: string;
    system_prompt?: string;
    temperature?: number;
    max_tokens?: number;
...
class DigitalTwinService {
    async createDigitalTwin(userId: string, twinData: CreateDigitalTwinParams): Promise<DigitalTwin> {
        try {
            const {
                name,
                profession,
        ...
        RETURNING *`,
            ...
            return result.rows[0] as DigitalTwin;
        } catch (error) {
    ...
    async updateDigitalTwin(twinId: string, userId: string, updates: UpdateDigitalTwinParams): Promise<DigitalTwin> {
        try {
            const allowedFields = [
                'name',
                'profession',
            ...
            (Object.keys(updates) as Array<keyof UpdateDigitalTwinParams>).forEach(key => {
    ...
    async getDigitalTwinByUserId(userId: string): Promise<DigitalTwin | null> {
        try {
            ...
            return result.rows[0] as DigitalTwin || null;
        } catch (error) {
    ...
    async addKnowledgeBaseEntry(twinId: string, entry: KnowledgeBaseEntry): Promise<KnowledgeBaseEntry> {
        try {
         ...
         RETURNING *`,
            ...
            await vectorStoreService.upsertEmbedding({
                id: result.rows[0].id,
                vector: embedding,
                metadata: { twinId, source: 'knowledge_base', title },
    ...
    async updateRAGConfig(twinId: string, config: RAGConfig): Promise<RAGConfig> {
        try {
         ...
         RETURNING rag_config`,
            ...
            logger.info(`RAG config updated for twin ${twinId}`);
